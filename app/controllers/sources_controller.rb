class SourcesController < ApplicationController
  ...
  def create
    ...
    if @project.save
      redirect_to organization_project_sources_path(@organization, @project, @source)
    else
      render :action => 'new'
    end
  end
  ...
  def update
    respond_to do |format|
      if @source.update_attributes(params[:source])
        format.html { redirect_to organization_project_sources_path(@organization, @project, :notice => 'source was successfully updated.') }
